...
project7=Project.create(name:"Meme Generator", like:Faker::Number.between(from: 5, to: 30 ), user_id:user2.id, description:"Create a Meme for yourself! very fun try it out!",link:"https://memeflip.netlify.app/",image:"https://i.imgflip.com/1ur9b0.jpg", category:"entertainment")
...
project11=Project.create(name:"Do I Have It?", like:Faker::Number.between(from: 5, to: 30 ), user_id:user2.id, description:"This app is a COVID-19 questionnaire to determine if you have the novel Corona virus.", link:"https://do-i-have-it.netlify.app", image:"https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTemsciblFfmgCPy0-zM4XTT4ZL8eSTm-LxLtJ_Szg-JFjFcWvZ&usqp=CAU", category:"Health")
...
project22=Project.create(name:"Portfolio", like:Faker::Number.between(from: 5, to: 30 ), user_id:user3.id, description:"My personal portfolio! hope you guys enjoy it!", link:"https://www.mazellier.design/", image:"https://assets.awwwards.com/awards/media/cache/optimize/submissions/2020/05/5eb680020015b205872200.jpg", category:"portfolio")
project23=Project.create(name:"Meal Finder App", like:Faker::Number.between(from: 5, to: 30 ), user_id:user1.id, description:"A Meal Finder app where you can search any meal's recipes (ingredients)", link:"https://vanillawebprojects.com/projects/meal-finder/", image:"https://www.themealdb.com/images/media/meals/wqqvyq1511179730.jpg", category:"Health")
project24=Project.create(name:"Personal Portfolio", like:Faker::Number.between(from: 5, to: 30 ), user_id:user2.id, description:"Personal Portfolio that I created", link:"http://ejosue.com/", image:"http://ejosue.com/wp-content/uploads/12471560_1239672876059393_3400546420150672993_o.jpg", category:"portfolio")
...
comment3=Comment.create(user_id:user2.id, project_id:project3.id, content:"Loving it! keep it up!")
comment4=Comment.create(user_id:user3.id, project_id:project4.id, content:"hahaha what a creative way to make this project")
comment5=Comment.create(user_id:user4.id, project_id:project5.id, content:"Love the styling! where do you get those from?")
comment6=Comment.create(user_id:user4.id, project_id:project6.id, content:"As a cryto guy of myself! this website is wonderful!! wollah")
...
comment9=Comment.create(user_id:user2.id, project_id:project9.id, content:"Damn how are you building this???")
comment10=Comment.create(user_id:user1.id, project_id:project10.id, content:"Jezz nice project! well deserved to get an award for this!")
...
comment15=Comment.create(user_id:user4.id, project_id:project13.id, content:"Not bad, work on the styling more please!")
comment16=Comment.create(user_id:user3.id, project_id:project14.id, content:"definitely need this in real life!")
comment17=Comment.create(user_id:user4.id, project_id:project15.id, content:"This is actually pretty fun game!")
comment18=Comment.create(user_id:user3.id, project_id:project16.id, content:"Very cool idea!!")
comment19=Comment.create(user_id:user1.id, project_id:project17.id, content:"Love it! Keep it up!!")
comment20=Comment.create(user_id:user2.id, project_id:project18.id, content:"Thank you for this!")
comment21=Comment.create(user_id:user3.id, project_id:project19.id, content:"This is perfect at this corona's moment...thank you so much!")
